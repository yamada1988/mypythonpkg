...
for num_index in range(20, 20+1):
    positions = []
    ims = []
    ...
    t_interval = 1000 #ps
    tb = int(t_interval/int(tint*0.10))
    nmax = int(t_max/(t_interval/0.10)) 
    print(nmax)
    for i in range(nmax):
        x_ = x[i*tb:(i+1)*tb]
        y_ = y[i*tb:(i+1)*tb]
        z_ = z[i*tb:(i+1)*tb]
        plt.plot(x_,y_,z_,ms=1.0,lw=1.0,c=cm.hsv(i/(nmax+1)), label='i={0:>3d}(ns)'.format(i))
    
    plt.legend()
    figname = 'trajectories/{0:04d}.eps'.format(num_index)
    plt.savefig(figname)
