...
#                        pass
        ...
        niterations = 1101 # number of samples to collect per alchemical state
        lambdas_q = [1.0e0, 0.90e0, 0.80e0, 0.70e0, 0.60e0, 0.50e0, 0.40e0, 0.30e0, 0.20e0, 0.10e0, 0.0e0]
        lambdas_v = [1.0e0, 0.90e0, 0.80e0, 0.70e0, 0.60e0, 0.50e0, 0.40e0, 0.30e0, 0.250e0, 0.20e0, 0.0150e0, 0.10e0, 0.050e0, 0.010e0, 0.0050e0, 0.0010e0, 0.00010e0, 0.000010e0, 0.00000010e0, 0.000000010e0, 0.0e0] # n=21
        nlamq = len(lambdas_q)
        nlamv = len(lambdas_v)
        ...
        simulation0.minimizeEnergy(tolerance = 1.0, maxIterations=10000)
            ...
            for iteration in range(niterations):
                ...
                for index in range(system.getNumParticles()):
                    if index in alchemical_particles:
                        nbforce.setParticleParameters(index, lambda_q*nb_params[index][0]*elementary_charge, nb_params[index][1]*nanometer, lambda_v*nb_params[index][2]*kilojoule/mole)
                ...
                simulation0.reporters.append(StateDataReporter(mdlog, 200, time=True,
        ...
        #        if k == 0 or k == nstates-2:
                ...
                if iteration < equiliter:
                    ...
                    for l in range(nstates-1):
                        ...
                        for index in range(system.getNumParticles()):
                            if index in alchemical_particles:
                                nbforce.setParticleParameters(index, lambda_ql*nb_params[index][0]*elementary_charge, nb_params[index][1], lambda_vl*nb_params[index][2]*kilojoule/mole)
                                print(nbforce.getParticleParameters(index))
                            else:
                                nbforce.setParticleParameters(index, nb_params[index][0]*elementary_charge, nb_params[index][1]*nanometer, nb_params[index][2]*kilojoule/mole)    
