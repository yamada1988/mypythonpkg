...
class MDConductor_vs:
    def __init__(self,sysindex):
    ...
    def loadFile(self, file):
        ...
        self.refs_freeze = InpDict['refs_freeze']
    ...
    def preparation(self, sysdir='SYS/', mddir='MD/'):
        ...
        # System input
        sysgro = sysdir + self.sysfile.split('.gro')[0] + self.sysindex + '.gro'
    ...
    def create_top(self, systop, gro):
        if self.pbc:
            ...
            #For virtual sites
            top = pmd.gromacs.GromacsTopologyFile(systop,xyz=gro.positions,box=gro.box)
            #top = GromacsTopFile(systop, periodicBoxVectors=gro.getPeriodicBoxVectors())
        else:
            top = pmd.gromacs.GromacsTopologyFile(systop,xyz=gro.positions)
    ...
    def create_system(self, top):
        ...
        if self.nptflag:
            ...
            MCB = MonteCarloBarostat(pressure, self.temperature)
            system.addForce(MCB)
        else:
            nptstep = 0       
        ...
        if self.ghost_particle:
            ...
            for i in ghost_particles:
                for j in core_particles:
                    ...
                    try:
                        ...
                        pass
                    try:
                        nonbonded_force.addException(j, i, chargeProd=slt_param[i][0]*slt_param[j][0],
                                                           sigma=0.50*(slt_param[i][1]+slt_param[j][1]),
                                                           epsilon=sqrt(slt_param[i][2]*slt_param[j][2]))
                    except:
                        pass
            ...
            for index in range(total_particles):
                ...
                if index in ghost_particles:
                    ...
                    #print('delete interaction index: {0:7d} for nbforce02.'.format(index))
            ...
            #energy_function = '-1.0*PI_ONE_EPS0*charge1*charge2/r -4.0*epsilon*x*(x-1.0); x = (sigma/r)^6;'
            #energy_function += 'sigma = 0.50*(sigma1+sigma2); epsilon = sqrt(epsilon1*epsilon2);'
            #custom_force = openmm.CustomNonbondedForce(energy_function)
    
            #custom_force.addGlobalParameter('PI_ONE_EPS0', 138.935456)
            ...
            #    nb_params.append(nonbonded_force.getParticleParameters(index))
            ...
            #custom_force.addInteractionGroup(core_particles, core_particles)

            #print('Set NonbondedMethod for CustomNonbonded interaction')
            #print('before:{0:d}'.format(custom_force.getNonbondedMethod()))
            #custom_force.setNonbondedMethod(1)
            #print('after:{0:d}'.format(custom_force.getNonbondedMethod()))
            #custom_force.setCutoffDistance(20.0e0 * nanometer)
            #custom_force.setUseSwitchingFunction(True)
            #custom_force.setSwitchingDistance(19.80e0 * nanometer)#nonbonded_switch)
...
#                print(nonbonded_force.getExceptionParameters(index))
    ...
    # EM simulation
    def minimize(self, simulation, emname, index, sysdir='SYS/', mddir='MD/',max_iter=5000):
        emname = emname + index
        ...
        simint = LangevinIntegrator(self.temperature, self.friction_const, self.dt)
        ...
        if self.fileformat == 'GRO':
            mdgro = mddir + emname + '.gro'
        elif self.fileformat == 'PDB':
            mdpdb = mddir + emname + '.pdb'
    ...
    def mdrun_refs(self, simulation, ensname, index, mddir='MD/', sysdir='SYS/', 
                   assert_system=True, check_eneflag=False, nrep='01', remdflag=False, niter=1, niter_tot=1):
        print('')
        freeze_steps = 100*self.recstep
        nofreeze_steps = 10*self.recstep
        int_steps = int(self.steps/(110*self.recstep))
        self.steps = int_steps * (freeze_steps + nofreeze_steps)
        print(ensname+' Simulation:', 'self.steps:', self.steps, 'mdresctep:', self.recstep)
        mdname = ensname + index
        mdlog = mddir + mdname + '.log'
        self.recstep = int(self.recstep/1)
        log_f = open(mdlog, mode='a+')
        log_reporter = StateDataReporter(log_f, self.recstep, time=True,
                                                              totalEnergy=True, potentialEnergy=True, temperature=True, density=True,
                                                              progress=True, remainingTime=True, speed=True, totalSteps=niter_tot*self.steps, separator='\t')
        ...
        masses = [0.0 for i in range(numatm)]
        for i in range(numatm):
            masses[i] = simsys.getParticleMass(i)

        # freeze-refs md
        for it in range(int_steps):
            for i in range(numatm):
                if i not in self.ghosts:
                    simsys.setParticleMass(i, 0.0)
            simulation.step(freeze_steps)
            simsys = simulation.system
            for i in range(numatm):
                #print(i,masses[i])
                simsys.setParticleMass(i, masses[i])
            simulation.step(nofreeze_steps)
...
class REMDConductor_vs(MDConductor_vs, object):
    def __init__(self, T_list, mode=None):
        super(REMDConductor_vs, self).__init__()
        if mode == 'REMD':
